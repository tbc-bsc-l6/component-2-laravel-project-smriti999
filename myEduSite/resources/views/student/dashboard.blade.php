...
<div class="min-h-screen bg-gray-100 flex">
    ...
    <aside class="w-64 bg-white shadow-md">
        ...
        <nav class="mt-6">
            <a href="{{ route('student.dashboard') }}" class="block py-2 px-6 hover:bg-gray-200">Dashboard</a>
        </nav>
    </aside>
    ...
    <main class="flex-1 p-6">
        ...
        <section class="mb-8">
            ...
            @forelse($completedModules as $module)
                <div class="bg-white shadow rounded p-4 mb-2">
                    ...
                    <p>Status: 
                        @if($module->pivot->pass_status == 'passed')
        ...
        <section>
            ...
            @forelse($availableModules as $module)
                <form method="POST" action="{{ route('student.enroll', $module->id) }}" class="mb-2 flex justify-between items-center bg-white shadow p-4 rounded">
                    @csrf
                    <div>{{ $module->module }}</div>
                    <button type="submit" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Enroll</button>
                </form>
            @empty
